...
impl<T: Config> Pallet<T> {
	...
	fn do_create_inherent(data: ParachainInherentData) -> Call<T> {
		...
		// DMQ.
		// FIX: za svež para (LastProcessedDownwardMessage=None, LastRelayChainBlockNumber=0)
		// NE brišemo poruke — sve genesis DMP poruke moraju biti prosleđene runtime-u.
		// Original unwrap_or({sent_at:0, reverse_idx:0}) pogrešno briše genesis poruke
		// (sent_at=0) što uzrokuje DMQ head mismatch panic.
		if let Some(last_processed_msg) = LastProcessedDownwardMessage::<T>::get() {
			downward_messages.drop_processed_messages(&last_processed_msg);
		} else if last_relay_block_number > 0 {
			let fallback =
				InboundMessageId { sent_at: last_relay_block_number, reverse_idx: 0 };
			downward_messages.drop_processed_messages(&fallback);
		}
		// else: svež para — ne brišemo ništa
		let mut size_limit = messages_collection_size_limit;
		let downward_messages = downward_messages.into_abridged(&mut size_limit);

		// HRMP — isti fix.
		if let Some(last_processed_msg) = LastProcessedHrmpMessage::<T>::get() {
			horizontal_messages.drop_processed_messages(&last_processed_msg);
		} else if last_relay_block_number > 0 {
			let fallback =
				InboundMessageId { sent_at: last_relay_block_number, reverse_idx: 0 };
			horizontal_messages.drop_processed_messages(&fallback);
		}
