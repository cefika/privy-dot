#!/usr/bin/env bash
# Starts zombienet and automatically injects Aura keys into both collators.
# Usage: ./start-zombienet.sh
set -euo pipefail
...
insert_aura_key() {
    ...
    echo -n "Injecting $name Aura key..."
    local result
...
echo ""
echo -n "Starting eth-rpc proxy on http://127.0.0.1:8545..."
eth-rpc \
